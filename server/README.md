# Thread Clone - GraphQL Server

This is the code for the **GraphQL server API** of the **Thread clone** web app with following stacks:

- [**Apollo Server**](https://github.com/apollographql/apollo-server): HTTP server for GraphQL APIs
- [**GraphQL Nexus**](https://nexusjs.org/docs/): GraphQL schema definition and resolver implementation
- [**Prisma Client**](https://www.prisma.io/docs/concepts/components/prisma-client): Databases access (ORM)
- [**Prisma Migrate**](https://www.prisma.io/docs/concepts/components/prisma-migrate): Database migrations
- [**PostgreSQL**](https://www.postgresql.org/): The relational SQL database
...
Run the following command to create your Postgres database file. This also creates tables that are defined in [`prisma/schema.prisma`](./prisma/schema.prisma):

```
npx prisma migrate dev --name init
```

**Notes**: When `npx prisma migrate dev` is executed against a newly created database, seeding is also triggered. The seed file in [`prisma/seed.js`](./prisma/seed.js) will be executed and your database will be populated with the seeded data.

### 3. Prepare the schema

If you don't familiar with the Nexus library's syntax, follow these steps to shorten the learning & implementation time

1. Define the GraphQL schema like normal (e.g., const typeDefs = gql`{your schema here}`)
   ```
   import {gql} in "graphql-tag"
   const typeDefs = gql`<your schema here>`
   ```
2. Copy and paste your schema (the content between the gql`` literal) into [Nexus SDL Converter](https://nexusjs.org/converter). This will convert your schema into the Nexus schema file
3. Copy the generated Nexus schema into the [`src/schema.js`](./src/schema.js)

_Note_: GraphQL Scalars is a library of custom GraphQL scalar types for creating precise type-safe GraphQL schemas.
...
Navigate to [http://localhost:4000](http://localhost:4000) in your browser to explore the API of the GraphQL server
...
If you want to try with another database, you can adjust the the database connection in [`prisma/schema.prisma`](./prisma/schema.prisma) by reconfiguring the `datasource` block.
